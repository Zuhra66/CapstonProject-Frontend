...
async function authedJson(
  ...
  if (match && ["POST", "PUT", "PATCH", "DELETE"].includes(upper)) {
    csrfHeader = { "X-XSRF-TOKEN": decodeURIComponent(match[1]) };
  }
...
const updateEditForm = (field, value) => {
  // 🔧 CHANGE: Member → User triggers warning only
  ...
  // 🔧 CHANGE: User → Member opens assign modal (NO API here)
...
const getMembershipType = (user) => {
  ...
  if (planName?.includes("student")) label = "Student";
  if (planName?.includes("general")) label = "General";
  ...
  <div className="card-body">
    <div className="row g-3 align-items-center"> {/* Changed from align-items-end to align-items-center */}
                  ...
                  {filteredUsers.length === 0 ? (
                    ...
                    filteredUsers.map((user) => (
                      <tr key={user.id} className={editingId === user.id ? 'table-active' : ''} style={{ borderColor: '#ADBBDA' }}>
                        ...
                        <td style={{ padding: '0.5rem' }}>{getMembershipType(user)}</td>
      ...
      >
        ...
        <Modal.Body>
          <p>
            You are assigning <strong>{assignUser?.email}</strong> as a
          ...
          <div className="alert alert-warning mt-3" style={{ fontSize: "0.85rem" }}>
            This is a manual admin override. No billing will occur.
          </div>
        </Modal.Body>
        ...
        <Modal.Footer>
          ...
          // Assign membership = set role only
          <Button
            variant="primary"
            ...
            onClick={async () => {
              try {
                ...
                setEditForm(prev => ({ ...prev, role: "Member" }));
